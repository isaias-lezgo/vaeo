...
pnpm verify:assignment   # lib/assignment-funnel.ts — precedencia sin-asesor sobre estatus
...
- **`lost-by-dimension-chart.tsx` ("Leads no ganados por servicio") es el espejo de
  ...
  **Perdidas ⇄ No ganadas** (esta última suma las abiertas, así que los meses recientes se
  ...
  el switch repinten las series. Su segmento **"Sin servicio" domina** (medido el
  2026-08-23: 87% de las no ganadas de VAEO, 74% de las de MESH) porque `Servicio` se
  captura al **perfilar** el lead —0/32 en Nuevo Lead, 1/49 en Lead en proceso, 12/13 en
  Lead Perfilado, 100/100 en ganadas, 1/100 en perdidas— y los leads que se caen mueren
  antes de llegar ahí. La nota calculada bajo la gráfica lo dice con todas sus letras;
  **no escondas la cubeta**, misma regla que en `lost-cross-matrix.tsx`.
...
| `lib/assignment-funnel.ts` | el apilado mensual leads-sin-asesor vs. desenlace; **sin asesor gana sobre el estatus** |
